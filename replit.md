> NAYVO is now maintained and released from main. See PROJECT_GUIDE.md and RELEASE.md. Replit setup below is historical.
...
### آلية التذاكر
```
عدد الفرص = floor( (قيمة المنتجات − الخصم) ÷ سعر الفرصة )
```
رسوم التوصيل تُضاف للمبلغ المستحق لكنها **لا تدخل** في احتساب الفرص.
...
## نظام التصميم — المرجع الثابت
المصدر الوحيد للألوان والمقاسات: `constants/colors.ts`. لا تُكتب ألوان يدوياً في الشاشات.

**القاعدة البصرية:** الأزرق للأفعال، الذهبي للسحب والجوائز، والأخضر للتأكيد فقط.

| الاستخدام | اللون | الكود |
|---|---|---|
| الأزرار والروابط والعنصر المحدد | أزرق ملكي | `#155EEF` |
| ضغط الزر الأساسي | أزرق داكن | `#1249BF` |
| خلفية الاختيار والأزرار الثانوية | أزرق فاتح جداً | `#EFF4FF` |
| الشعار والعناوين الرئيسية | كحلي | `#10224D` |
| الجائزة وأيقونة الهدية | ذهبي | `#F5A623` |
| خلفية رسائل فرص السحب | ذهبي فاتح | `#FFF4D6` |
| النص فوق الخلفية الذهبية | بني داكن | `#754500` |
| خلفية التطبيق | رمادي بارد فاتح | `#F6F8FC` |
| البطاقات والحقول وشريط التنقل | أبيض | `#FFFFFF` |
| النص الأساسي | رمادي داكن | `#182230` |
| النص الثانوي | رمادي | `#475467` |
| النص المساعد وغير المحدد | رمادي متوسط | `#667085` |
| الحدود والفواصل | رمادي فاتح | `#D0D5DD` |

**الحالات** (`StatusColors`) — كل حالة معها كلمة وأيقونة، ما بتعتمد على اللون وحده:
نجاح `#067647`/`#ECFDF3` · تنبيه `#B54708`/`#FFFAEB` · خطأ `#B42318`/`#FEF3F2` ·
معلومة `#175CD3`/`#EFF8FF` · غير متاح `#667085`/`#EAECF0`

**الخط:** Tajawal — عادي للنصوص، متوسط للأزرار، عريض للعناوين والأسعار.
المقاسات: عنوان رئيسي 26، عنوان قسم 20، عنوان فرعي 17، نص وأزرار 16، تفاصيل 14، شارات 12.

**الأشكال:** الأزرار ارتفاع 52 وزوايا 12 · البطاقات زوايا 16 · الحقول ارتفاع 52 ·
الهوامش الجانبية 16 · المسافات مضاعفات 4 · الأيقونات 24 من Ionicons.
...
components/
  ui.tsx                مكوّنات نظام التصميم المشتركة
  ProductCard.tsx       بطاقة منتج
  DrawBanner.tsx        بطاقة الجائزة الكحلية + عدّاد السحب
lib/
...
## Database Tables
`users`, `products` (مع `specs_json`), `draws`, `orders` (مع `delivery_fee`),
`order_items`, `tickets`,
